...
xml;
        $xml  = simplexml_load_string( $coments );
        $keys = array();
        $sqls = [];
        foreach ( $xml as $item ) {
            $sqls[] = "insert into comentaris VALUES(NULL, \"$item->usuari\", \"$item->establiment\", \"$item->text\", \"$item->data\", \"$item->hora\");";
        }
        foreach ( $sqls as $sql ) {
            DB::insert( $sql );
        }
...
xml;
        ...
        $sqls = [];
        foreach ( $xml as $item ) {
            $sqls[] = "insert into usuaris VALUES(NULL, \"$item->nom\", \"$item->email\", \"$item->password\");";
        }
        foreach ( $sqls as $sql ) {
            DB::insert( $sql );
        }
